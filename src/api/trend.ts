...
const getTrendingList = async ({
	...
	const urlWithoutTag =
		...
		`?page=${pages}&size=${size}&startDate=${currentTime.toString()}&endDate=${monthLaterTime.toString()}`;
...
export const useGetTrendingList = () => {
	...
	const { data, fetchNextPage, hasNextPage } = useInfiniteQuery(
		...
		({ pageParam = 0 }) => {
			console.log(trending.tag);
			console.log(isListTag(trending.tag));
	...
	const [trendingList, fetchNextTrendingList, hasNextTrendingList] = [
		data,
		fetchNextPage,
		hasNextPage,
	];
	return { trendingList, fetchNextTrendingList, hasNextTrendingList };
};

...
import {
	DELETE_SCRAP_URL,
	EDIT_sCRAP_URL,
	GET_ARTICLE_SCRAP_URL,
	GET_LIST_SCRAP_URL,
	GET_OTHER_SCRAP_URL,
	GET_PRODUCT_SCRAP_URL,
	GET_VIDEO_SCRAP_URL,
...
export const uesGetProductScrap = async ({ pages, size }: fetchDatasProps) => {
	const scraps = await fetchDatas({
		url: GET_PRODUCT_SCRAP_URL,
		pages: pages,
		size: size,
	});
	return scraps;
...
export const useGetScrapByType = async ({
	pages,
	size,
	type,
}: fetchDatasProps & { type: string }) => {
	const scraps = await fetchDatas({
		url: findURLByType[type as keyof typeof findURLByType],
		pages: pages,
		size: size,
	});
	return scraps;
